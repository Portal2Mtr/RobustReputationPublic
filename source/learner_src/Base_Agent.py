...
class Base_Agent(object):
    ...
    def __init__(self, config):
        ...
        self.log_game_info()
        self.hyperparameters['batch_size'] = self.action_size
        self.var_est = 0.0
    ...
    def log_game_info(self):
        ...
        for ix, param in enumerate([self.environment_title, self.action_types, self.action_size, self.lowest_possible_episode_score,
                                    ...
                                    self.device]):
            test = 0 #self.logger.info("{} -- {}".format(ix, param))
    ...
    def update_learning_rate(self, starting_lr,  optimizer):
        ...
        if len(self.rolling_results) > 0:
            ...
            for g in optimizer.param_groups:
                g['lr'] = new_lr
        if random.random() < 0.001: test = 0 #self.logger.info("Learning rate {}".format(new_lr))
    ...
    def save_experience(self, memory=None, experience=None):
        ...
        if experience is None: experience = {'state':self.state,
                                             'action':self.action,
                                             'reward':self.reward,
                                             'next_state':self.next_state,
                                             'done':self.done}
        memory.add_experience(*experience.values())
    ...
    def import_experience(self,experience):
        ...
        if experience is not None:
            self.memory.add_experience(*experience.values())
